...
| **播放狀態改為 2 秒；佇列仍維持 1 秒** | **90 req/min** | 新增待改方案；播放狀態由 60 降為 30 req/min，但佇列仍為 60 req/min，總流量降低 25%。需將兩支 API 拆開排程。 |
| 播放狀態與佇列皆改為 2 秒 | 60 req/min | 總流量降低 50%，但仍會不必要地頻繁查詢佇列。 |
...
**已註記待改**：優先評估「僅將 `getPlaybackState` 改為每 2 秒一次，
`getUsersQueue` 暫維持每秒一次」；預期總 Spotify 請求量由 120 降至 90 req/min
（25%）。實作時需拆分目前共用的 `pollSingle()` 輪詢排程。
